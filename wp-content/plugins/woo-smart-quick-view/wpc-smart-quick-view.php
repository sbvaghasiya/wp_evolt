...
! defined( 'WOOSQ_VERSION' ) && define( 'WOOSQ_VERSION', '3.3.4' );
			...
			class WPCleverWoosq {
				...
				function __construct() {
					...
					add_action( 'before_woocommerce_init', function () {
						if ( class_exists( '\Automattic\WooCommerce\Utilities\FeaturesUtil' ) ) {
							\Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility( 'custom_order_tables', WOOSQ_FILE, true );
						}
					} );
				}
									...
									if ( ! empty( $thumb_ids ) ) {
										foreach ( $thumb_ids as $thumb_id ) {
											if ( self::get_setting( 'content_image_lightbox', 'no' ) !== 'no' ) {
												...
												echo '<div class="thumbnail" data-id="' . $thumb_id . '">' . wp_get_attachment_image( $thumb_id, $image_size, false, array(
                                    ...
                                    <table class="form-table">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <p><label><input type="checkbox"
                                                                 name="woosq_settings[related_products][]"
                                                                 value="up-sells" <?php echo esc_attr( in_array( 'up-sells', $suggested ) ? 'checked' : '' ); ?>/> <?php esc_html_e( 'Up-Sells products', 'woo-smart-quick-view' ); ?>
                                                    </label></p>
                                                <p><label><input type="checkbox"
                                                                 name="woosq_settings[related_products][]"
                                        ...
                                        <tr class="submit">
                                            <th colspan="2">
												<?php settings_fields( 'woosq_settings' ); ?>
												<?php submit_button(); ?>
                                            </th>
                                        </tr>
                                    </table>
				...
				function admin_enqueue_scripts( $hook ) {
					...
					if ( strpos( $hook, 'woosq' ) ) {
						...
						wp_enqueue_script( 'fonticonpicker', WOOSQ_URI . 'assets/libs/fonticonpicker/js/jquery.fonticonpicker.min.js', array( 'jquery' ) );
				...
				function enqueue_scripts() {
					...
					wp_localize_script( 'woosq-frontend', 'woosq_vars', array(
							...
							'related_slick_params'    => apply_filters( 'woosq_related_slick_params', json_encode( apply_filters( 'woosq_related_slick_params_arr', array(
								'slidesToShow'   => 2,
								'slidesToScroll' => 2,
								'dots'           => true,
								'arrows'         => false,
							...
							'thumbnails_slick_params' => apply_filters( 'woosq_thumbnails_slick_params', json_encode( apply_filters( 'woosq_thumbnails_slick_params_arr', array(
								'slidesToShow'   => 1,
								'slidesToScroll' => 1,
								'dots'           => true,
								'arrows'         => true,
							...
							'thumbnails_zoom_params'  => apply_filters( 'woosq_thumbnails_zoom_params', json_encode( apply_filters( 'woosq_thumbnails_zoom_params_arr', array(
								'duration' => 120,
								'magnify'  => 1
							) ) ) ),
						)
				...
				function wpcsm_locations( $locations ) {
					$locations['WPC Smart Quick View'] = array(
