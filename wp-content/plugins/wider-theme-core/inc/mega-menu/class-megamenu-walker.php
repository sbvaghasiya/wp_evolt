...
class EFramework_Mega_Menu_Walker extends Walker_Nav_Menu
{
    ...
    {
        $item_html = '';
        $megamenu = apply_filters('evolt_enable_megamenu', false);
        ...
        $extra_menu_custom = apply_filters("evolt_menu_edit", array());
        if ($item->evolt_onepage === 'is-one-page' && !wp_script_is('single-page-nav')) {
            wp_register_script('single-page-nav', EVOLT_URL . 'assets/js/lib/jquery.singlePageNav.js', array('jquery', 'elementor-waypoints'), 'all', true);
        ...
        add_filter('nav_menu_link_attributes', function ($atts, $item) {
            if (isset($item->evolt_onepage) && $item->evolt_onepage === 'is-one-page') {
                if(!isset($atts['class']) || empty($atts['class'])){
                    if (isset($item->evolt_custom_class) && !empty($item->evolt_custom_class)) {
                        $atts['class'] = 'is-one-page '.$item->evolt_custom_class;
                    } else {
            ...
            if (isset($item->evolt_onepage_offset)) {
                $atts['data-onepage-offset'] = $item->evolt_onepage_offset;
            }

            if (isset($item->evolt_custom_class) && !empty($item->evolt_custom_class)) {
                if(!isset($atts['class']) || empty($atts['class'])){
                    $atts['class'] = $item->evolt_custom_class;
                }
            }

            return $atts;
        }, 10, 2);
        if (!empty($item->evolt_megaprofile) && $megamenu) {
        ...
        $item->evolt_icon_position = 'left';
        if (!empty($item->evolt_icon)) {
            if ('left' === $item->evolt_icon_position) {
                $args->old_link_before = $args->link_before;
                $args->link_before = '<span class="link-icon left-icon"><i class="' . esc_attr($item->evolt_icon) . '"></i></span>' . $args->link_before;
            } else {
                $args->old_link_after = $args->link_after;
                $args->link_after = $args->link_after . '<span class="link-icon right-icon"><i class="' . esc_attr($item->evolt_icon) . '"></i></span>';
            }
        }

        /* Marker */
        if (isset($item->evolt_menu_marker) && !empty($item->evolt_menu_marker)) {
            $args->old_link_after = $args->link_after;
            $args->link_after = '<cite class="evolt-menu-item-marker">' . esc_attr($item->evolt_menu_marker) . '</cite>' . $args->link_after;
        }
        ...
        if (!empty($item->evolt_megaprofile)) {
            $item_html .= $this->get_megamenu($item->evolt_megaprofile);
        }
