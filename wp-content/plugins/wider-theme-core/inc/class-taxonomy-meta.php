...
class EVOLT_Taxonomy_Meta
{
    ...
    function __construct()
    {
        ...
        do_action('evolt_taxonomy_meta_register', $this);
    ...
    function admin_init()
    {
        ...
        if ('term.php' == $pagenow) {
            $this->notices = get_transient('evolt-taxonomy-meta-transients');

            // If the transient exists then we override it with an empty value
            if (false !== $this->notices) {
                set_transient('evolt-taxonomy-meta-transients', '');
            } else {
    ...
    {
        $this->default_args['opt_name'] = isset($this->default_args['opt_name']) ? $this->default_args['opt_name'] . '_taxonomy_metabox' : 'evolt_taxonomy_metabox';
    ...
    function add_section($taxonomy, $section = array())
    {
        ...
        foreach ($section['fields'] as $fkey => $field) {
            ...
            if (in_array($field['id'], $this->field_ids[$taxonomy])) {
                trigger_error(sprintf(esc_html__('The field with id %1$s for taxonomy %2$s is already registered.', EVOLT_TEXT_DOMAIN), esc_html($field['id'], ETCTEXT_DOMAIN), esc_html($taxonomy, ETCTEXT_DOMAIN)));
        ...
        if (!empty($section['id'])) {
            if ($this->section_exist($section['id'], $taxonomy)) {
                trigger_error(sprintf(esc_html__('Section %1$s for taxonomy %2$s is already exist.', EVOLT_TEXT_DOMAIN), esc_html($field['id']), esc_html($taxonomy)));
                return;
            } else {
    ...
    {
        wp_nonce_field('evolt_taxonomy_metabox_nonce_action', 'evolt_taxonomy_metabox_nonce');
    ...
    function panel_scripts()
    {
        ...
        wp_enqueue_script('evolt-metabox', EVOLT_URL . '/assets/js/metabox' . Redux_Functions::isMin() . '.js', array('jquery', 'redux-js'), self::$version, 'all');
    ...
    protected function save_term_meta($term_id, $tt_id, $taxonomy, $transient = false)
    {
        ...
        if (empty($_POST[$opt_name]) || empty($_POST['evolt_taxonomy_metabox_nonce']) || !wp_verify_nonce($_POST['evolt_taxonomy_metabox_nonce'], 'evolt_taxonomy_metabox_nonce_action')) {
            return;
        }
        ...
        if ($transient) {
            ...
            if (!empty($redux->errors) || !empty($redux->warnings)) {
                ...
                set_transient('evolt-taxonomy-metabox-transients', $notices);
            }
        }
    }
...
//        }
    ...
    protected function generate_output_css($taxonomy, $options)
    {
        ...
        echo '<style id="evolt-' . $taxonomy . '-dynamic-css" data-type="redux-output-css">' . $redux->outputCSS . '</style>';
    }
}
