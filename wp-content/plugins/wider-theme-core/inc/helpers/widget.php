...
if(!function_exists('evolt_generate_class_name')){
    function evolt_generate_class_name($name){
        ...
        $class_name = "EVOLT_" . $class_name . "_Widget";
        return $class_name;
    }
}

if(!function_exists('evolt_generate_file_class_name')){
    function evolt_generate_file_class_name($name){
        $name = strtolower($name);
        $name = replace_all_special_character($name, '-');
...
if(!function_exists('evolt_create_class_widget')){
    function evolt_create_class_widget($file_path, $class_name, $name, $title, $icon, $categories, $params, $styles = [], $scripts = []){
        $file_content_template_path = EVOLT_PATH . 'inc/widgets/class-template/class-widget-template.txt';
...
if(!function_exists('evolt_add_custom_widget')){
    function evolt_add_custom_widget($widget, $dir = EVOLT_PATH . 'inc/widgets/'){
        ...
        $class_name = evolt_generate_class_name($widget['name']);
        $file_name = evolt_generate_file_class_name($widget['name']);
        ...
        if(defined('DEV_MODE') && DEV_MODE){
            ...
            //     // unlink($file_path);
            // }
            $result = evolt_create_class_widget($file_path, $class_name, $name, $title, $icon, $categories, $params, $styles, $scripts);
        }
