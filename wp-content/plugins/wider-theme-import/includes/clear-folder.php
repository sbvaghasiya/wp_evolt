 ...
 * @create: 23-Nov-17
 */
function evolt_ie_clear_tmp(){

    $upload_dir = wp_upload_dir();

    evolt_ie_delete_directory($upload_dir['basedir'] . '/evolt-attachment-tmp');
    evolt_ie_delete_directory($upload_dir['basedir'] . '/evolt-ie-demo');
}

function evolt_ie_delete_directory($dir)
{
    if (!file_exists($dir)) {
        return true;
    }
...
function evolt_ie_import_finish($demo_id, $demo_path){
    global $wp_filesystem, $import_result;
    global $table_prefix, $wpdb;

    update_option('evolt_ie_demo_installed',$demo_id);
    ...
    $file = evolt_ie()->plugin_dir.'assets/log.txt';
