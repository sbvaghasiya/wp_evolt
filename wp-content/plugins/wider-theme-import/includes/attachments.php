...
function evolt_ie_media_export($folder_dir)
{
    global $wp_filesystem;

    $upload_dir = wp_upload_dir();

    $media_backup = $upload_dir['basedir'] . '/evolt-attachment-tmp/';
    ...
    if (class_exists('ZipArchive')){
        $zip = new ZipArchive;
        $zip->open($upload_dir['basedir'] . '/evolt-attachment-tmp.zip', ZIPARCHIVE::CREATE | ZipArchive::OVERWRITE);
        evolt_ie_zip_folder($media_backup, $zip);

        $zip->close();

        /* media */
        if (!is_dir($folder_dir . 'content')) wp_mkdir_p($folder_dir . 'content');
        $attachment = evolt_ie_export_wp(array('content' => 'attachment'));
        $wp_filesystem->put_contents($folder_dir . 'content/attachment-data.xml', $attachment, FS_CHMOD_FILE);
    }
}

function evolt_ie_media_import($options, $folder_dir , $manual_import)
{
    ...
    if (empty($options['attachment'])) {
        ...
        if($manual_import === false){
            ...
            /* download & unzip. */
            $_cache = trailingslashit($upload_dir['basedir'] . '/evolt-demo');

            if (!is_dir($_cache))
                wp_mkdir_p($_cache);

            wp_safe_remote_get($options['attachment'], array('timeout' => 3000, 'stream' => true, 'filename' => $_cache . 'evolt-attachment-tmp.zip'));

            unzip_file($_cache . 'evolt-attachment-tmp.zip', $upload_dir['basedir']);
        }
        $wp_import = new EVOLT_Import();
        /* import files. */
