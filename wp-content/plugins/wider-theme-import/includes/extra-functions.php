...
if ( ! function_exists( 'evolt_ie_export_demo_info' ) ) {
	function evolt_ie_export_demo_info( $file, $demo_info = array() ) {
		if ( ! empty( $demo_info ) ) {
...
if ( ! function_exists( 'evolt_ie_export_elementor_data' ) ) {
	function evolt_ie_export_elementor_data( $file, $elemetor_data = array() ) {
...
if ( ! function_exists( 'evolt_ie_import_elementor_data' ) ) {
	function evolt_ie_import_elementor_data( $file ) {
		if ( file_exists( $file ) ) {
...
if ( ! function_exists( 'evolt_ie_extra_options_export' ) ) {
	/**
	 ...
	 * @param array $options
	 */
	function evolt_ie_extra_options_export( $file, $options = array() ) {
		if ( ! empty( $options ) ) {
			global $wp_filesystem;
			$file_contents = array();
...
if ( ! function_exists( 'evolt_ie_extra_options_import' ) ) {
	/**
	 ...
	 * @param array $options
	 */
	function evolt_ie_extra_options_import( $file ) {
 ...
 */
function evolt_ie_process_demo_folder( $folder_name ) {

	if ( ! is_dir( evolt_ie()->theme_dir . $folder_name ) ) {
		wp_mkdir_p( evolt_ie()->theme_dir . $folder_name );
	}

	return trailingslashit( evolt_ie()->theme_dir . $folder_name );
}

function evolt_ie_replace_site_url( $contents, $folder_dir ) {
...
if(!function_exists('evolt_import_truncate_tables')){
	function evolt_import_truncate_tables(){
		$tables = apply_filters('evolt_import_truncate_tables', [
			'posts',
			'postmeta',
			'terms',
			'termmeta',
