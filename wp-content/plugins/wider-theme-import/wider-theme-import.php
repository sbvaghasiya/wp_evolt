<?php
/**
 * Plugin Name: Wider Theme Import
 ...
 * License: GPLv2
 * Text Domain: wider-theme-import
 */
if (!defined('ABSPATH')) {
    exit();
}
define('CTI_TEXT_DOMAIN', 'wider-theme-import');

if (!class_exists('EVOLT_Import_Export')) {

    /**
     * Main Class EVOLT_Import_Export
     *
     * @since 1.0.0
     *
     * @description: Public EVOLT_Import_Export:: or GLOBAL evolt_ie()
     *
     * @author: Wider-Themes
     *
     * @create: 15 November, 2017
     */
    class EVOLT_Import_Export
    {
        public $file;
        public $basename;
        public $plugin_dir;
        ...
        /**
         * @return EVOLT_Import_Export
         */
        public static function instance()
        {
            if (is_null(self::$instance)) {
                self::$instance = new EVOLT_Import_Export();
        ...
        function evolt_ie_menu_handle()
        {
            ...
            if (class_exists('Wider_Theme_Core')) {
                add_submenu_page($this->theme_text_domain, esc_html__('Import Demo', CTI_TEXT_DOMAIN), esc_html__('Import Demo', CTI_TEXT_DOMAIN), 'manage_options', 'evolt-import', array($this, 'evolt_import_demo_page'));
            } else {
                add_submenu_page('tools.php', esc_html__('Import Demo', CTI_TEXT_DOMAIN), esc_html__('Import Demo', CTI_TEXT_DOMAIN), 'manage_options', 'evolt-import', array($this, 'evolt_import_demo_page'));
            }
        }

        public function evolt_import_demo_page()
        {
            $export_mode = $this->evolt_ie_enable_export_mode();
            include_once evolt_ie()->plugin_dir . 'templates/import-page.php';
        }


        function evolt_ie_enable_export_mode()
        {
            return apply_filters('evolt_ie_export_mode', false);
        }
        ...
        private function includes()
        {
            ...
            add_action('admin_menu', array($this, 'evolt_ie_menu_handle'),100);
            add_action('admin_enqueue_scripts', array($this, 'evolt_ie_enqueue_scripts'));
            ...
            require evolt_ie()->plugin_dir . 'includes/api/parsers.php';

            /* class WP_Import not exists */
            if (!class_exists('EVOLT_Import'))
                require_once evolt_ie()->plugin_dir . 'includes/api/wordpress-importer.php';
        ...
        function evolt_ie_enqueue_scripts()
        {
            if (isset($_REQUEST['page']) && $_REQUEST['page'] === 'evolt-import') {
                wp_enqueue_style('evolt-ie.css', $this->plugin_url . 'assets/evolt-ie.css');
                wp_enqueue_script('evolt-ie.js', $this->plugin_url . 'assets/evolt-ie.js', array(), 'all', true);
            }
        }
    }
    ...
    function evolt_ie()
    {
        return EVOLT_Import_Export::instance();
    }

    $GLOBALS['evolt_ie'] = evolt_ie();
}