...
if (! class_exists ( 'Case_Theme_User_admin' )) {
	...
	class Case_Theme_User_admin {  
		...
		function __construct() {
			...
			add_action( 'evolt-user-form/inc/admin/tab/content', array(
				$this,
		...
		function register_plugin_settings() {
			...
			register_setting('evolt-user-form-general-group', 'user_press_layout');
            register_setting('evolt-user-form-general-group', 'user_press_bg_color');
            register_setting('evolt-user-form-general-group', 'user_press_bg_img');
            
            /* reservation options Email . */
            
            register_setting('evolt-user-form-email-group', 'user_press_subject_email');
            register_setting('evolt-user-form-email-group', 'user_press_email_send');
            register_setting('evolt-user-form-email-group', 'evolt-user-form-conten-email');
		...
		function plugin_action_links( $links ){
			...
			$action_links = array(
                 'settings' => '<a href="' . admin_url( 'users.php?page=evolt-user-form_admin' ) . '" title="' . esc_attr( esc_html__( 'View Users Press Settings', 'evolt-user-form' ) ) . '">' . esc_html__( 'Settings', 'evolt-user-form' ) . '</a>',
			);
		...
		function add_admin_page() {
			add_users_page ( esc_html__ ( 'Users Press', 'evolt-user-form' ), esc_html__ ( 'Users Press', 'evolt-user-form' ), 'manage_options', 'evolt-user-form_admin', array (
				$this,
				'add_admin_page_main'
			) );
		}
		...
		function add_admin_page_main() {
			...
			$tabs = array (
				'email' => esc_html__('Email', 'evolt-user-form'),
			);

			$tabs = apply_filters('evolt-user-form/admin/tabs', $tabs);

			?>
			<h1><?php esc_html_e('Wider Theme User', 'evolt-user-form'); ?></h1>
			<p><?php esc_html_e('A wordpress user manager plugin.', 'evolt-user-form'); ?></p>
				...
				<form id="mainform" method="post" action="options.php">
					...
					<h2 class="nav-tab-wrapper woo-nav-tab-wrapper">
						<?php foreach ($tabs as $key => $tab): ?>
							<a href="<?php echo admin_url( 'users.php?page=evolt-user-form_admin&tab=' . $key ); ?>" class="nav-tab<?php echo ( $current_tab == $key ? ' nav-tab-active' : '' ) ; ?>"><?php echo esc_html($tab); ?></a>
						<?php endforeach; ?>
					</h2>

					<?php  do_action('evolt-user-form/inc/admin/tab/content'); ?>
		...
		function add_admin_tab_content() {
			...
			$tab = apply_filters('evolt-user-form/inc/admin/tab/template', userpress()->plugin_dir . "admin/html_tab_$current_tab.php");

			if(!file_exists($tab)) return ;

			settings_fields( "evolt-user-form-$current_tab-group" );
			do_settings_sections( "evolt-user-form-$current_tab-group" );
		 ...
		 */
            private function option_color($options){
                    ...
                    ?>
                    <tr valign="top" class="evolt-user-form-option-color">
                            <th scope="row" class="titledesc">
                                    <label for="<?php echo esc_attr( $options['id'] ); ?>"><?php echo esc_html( $options['title'] ); ?></label>
                            </th>
                            <td class="forminp">
            ...
            private function option_image($options){
                    ...
                    $option_value = get_option( $options['id'], $options['default'] );
                            ...
                            ?>
                            <tr valign="top" class="evolt-user-form-option-media">
                                    <th scope="row" class="titledesc">
                                            <label for="<?php echo esc_attr( $options['id'] ); ?>"><?php echo esc_html( $options['title'] ); ?></label>
                                            <td class="forminp">
            ...
            private function option_layout($options){
                    ...
                    ?>
                    <tr valign="top" class="evolt-user-form-option-layout">
                                    <th scope="row" class="titledesc">
                                            <label for="<?php echo esc_attr( $options['id'] ); ?>"><?php echo esc_html( $options['title'] ); ?></label>
                                            <td class="forminp">
                                                    <ul>
         ...
         private  function option_switch($options)
         {
         	...
         	?>
         	<tr valign="top" class="evolt-user-form-option-switch">
				<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $options['id'] ); ?>"><?php echo esc_html( $options['title'] ); ?></label>
				</th>
				<td class="forminp">
