<?php
    ...
    foreach ($tabs as $key => $tab){
        ...
        if($tab['content_type'] == 'form') {
            $form_class = 'evolt-contact-form-layout1 style8';
    ...
    switch ($tab_type) {
        case 'horizontal': ?>
            <div class="evolt-tabs evolt-tabs--horizontal1 type-<?php echo esc_attr($tab_type); ?> <?php echo esc_attr($tab_style); ?>">
                <div class="evolt-tabs-title">
                    <?php foreach ($tabs as $key => $tab) : 
                        $icon_key = $widget->get_repeater_setting_key( 'evolt_icon', 'icons', $key );
                        $has_icon = ! empty( $tab['evolt_icon'] );
                        $widget->add_render_attribute( $icon_key, [
                            'class' => $tab['evolt_icon'],
                            'aria-hidden' => 'true',
                        ] );
                        ?>
                        <span class="evolt-tab-title <?php if($active_tab == $key + 1) { echo 'active'; } ?>" data-target="#<?php echo esc_attr($html_id.'-'.$tab['_id']); ?>">
                            <?php
                                if($is_new):
                                    \Elementor\Icons_Manager::render_icon( $tab['evolt_icon'], [ 'aria-hidden' => 'true' ] );
                            ?>
                            <?php else: ?>
                                <i <?php evolt_print_html($widget->get_render_attribute_string( $icon_key )); ?>></i>
                            <?php endif; ?>
                            <?php echo evolt_print_html($tab['tab_title']); ?>        
                        </span>
                    <?php endforeach; ?>
                </div>
                ...
                <div class="evolt-tabs-content">
                    <?php foreach ($tabs_content as $content_key => $tab_content): ?>
                        <div <?php evolt_print_html($widget->get_render_attribute_string( $content_key )); ?>><?php evolt_print_html($tab_content); ?></div>
                    <?php endforeach; ?>
                </div>
            </div>
            <?php break;
