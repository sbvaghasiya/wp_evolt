...
if(!empty($accordions)) : ?>
    <div id="<?php echo esc_attr($html_id); ?>" class="evolt-accordion layout1 <?php echo esc_attr($evolt_animate); ?>" data-wow-duration="1.2s">
        ...
        ?>
            <div class="evolt-accordion-item <?php echo esc_attr($is_active?'active':''); ?>">
                <<?php evolt_print_html($title_html_tag); ?> class="evolt-ac-title <?php echo esc_attr($is_active?'active':''); ?>" data-target="<?php echo esc_attr('#' . $_id.$html_id); ?>">
                    <a <?php evolt_print_html($widget->get_render_attribute_string( $title_key )); ?>><?php echo wp_kses_post($ac_title); ?></a>
