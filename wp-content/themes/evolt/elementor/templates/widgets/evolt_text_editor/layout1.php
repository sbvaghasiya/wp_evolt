...
$widget->add_render_attribute( 'text_editor', 'class', [ 'evolt-text-editor', 'elementor-clearfix' ] );

$widget->add_inline_editing_attributes( 'text_editor', 'advanced' );
?>
<div id="<?php echo esc_attr($html_id); ?>" class="evolt-text-editor <?php echo esc_attr($settings['evolt_animate']); ?>" data-wow-delay="<?php echo esc_attr($evolt_animate_delay); ?>ms">
	<div class="evolt-inline-css"  data-css="
        <?php if( !empty($text_color_gradient) && $gradient_direction == 'vertical' ) : ?>
            #<?php echo esc_attr($html_id) ?> .evolt-text-editor {
